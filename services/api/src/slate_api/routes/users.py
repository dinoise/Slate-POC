...
@router.post(
    "/", response_model=UserRead, status_code=status.HTTP_201_CREATED, summary="Create new user"
)
...
@router.get("/", response_model=PaginatedResponse[UserRead], summary="Get all users")
async def get_users(
    service: UserServiceDep,
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=1000),
) -> PaginatedResponse[UserRead]:
    items, total = await service.get_all_users(skip, limit)
